...
addEmployee: (state, action) => {
  ...
  (
    <div>
      ...
      <PDFDownloadLink
        document={<ItemCardPDF data={data} />} // Передаем данные в PDF
        fileName="item-card.pdf"
      >
        {({ loading }) => (
          <button className="cursor-pointer px-1 text-white bg-[var(--main-bg-blue)]">
            {loading ? "Загрузка..." : "Завантажити PDF"}
          </button>
        )}
      </PDFDownloadLink>
    </div>
...
import TimesNewRoman from "../../fonts/TimesNewRomanMTStd.ttf"; // путь к файлу
